...
const submit = () => {
    ...
    form.post(route('admin.blogs.store'), { forceFormData: true });
};
            ...
            <form class="space-y-6" @submit.prevent="submit">
                <div class="space-y-5 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
                    <div class="grid gap-5 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Short Description</label>
                            <textarea v-model="form.short_description" rows="2" :class="[inputClass, 'h-auto py-2', form.errors.short_description && 'border-red-500']"></textarea>
                            <p v-if="form.errors.short_description" class="mt-1 text-sm text-red-500">{{ form.errors.short_description }}</p>
                        </div>
                        <div>
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Category</label>
                            <input v-model="form.category" type="text" placeholder="e.g. Buying Guides" :class="inputClass" />
                            <p v-if="form.errors.category" class="mt-1 text-sm text-red-500">{{ form.errors.category }}</p>
                        </div>
                        <div>
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Tags (comma separated)</label>
                            <input v-model="tagsInput" type="text" placeholder="trucks, japan, import" :class="inputClass" />
                            <p v-if="form.errors.tags" class="mt-1 text-sm text-red-500">{{ form.errors.tags }}</p>
                        </div>
                        <div>
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Status</label>
                        ...
                        <div>
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Cover Image</label>
                            <label class="mt-1 flex h-11 cursor-pointer items-center gap-2 rounded-xl border border-dashed border-zinc-300 px-3 text-sm text-zinc-500 transition-colors hover:border-[#8e2527] dark:border-zinc-700">
                                <ImageIcon class="h-4 w-4" /> {{ form.cover_image ? form.cover_image.name : 'Upload cover…' }}
                ...
                <div class="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
                    <label class="mb-2 block text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Content</label>
                ...
                <div class="space-y-5 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
                    <h3 class="text-[13px] font-semibold text-zinc-500">SEO (optional)</h3>
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Meta Title</label>
                            <input v-model="form.meta_title" type="text" :class="inputClass" />
                        </div>
                        <div>
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Meta Keywords</label>
                            <input v-model="form.meta_keywords" type="text" :class="inputClass" />
                        </div>
                        <div class="sm:col-span-2">
                            <label class="text-[13px] font-medium text-zinc-700 dark:text-zinc-300">Meta Description</label>
                ...
                <button
                    ...
                    class="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#8e2527] font-medium text-white transition-colors hover:bg-[#a32c2f] disabled:opacity-60"
                >
                    <Loader2 v-if="form.processing" class="h-4 w-4 animate-spin" />
                    {{ form.publish_status === 'published' ? 'Publish Post' : 'Save Draft' }}
                </button>
            </form>
        </div>
    </AdminLayout>
