        ...
        <div class="mx-auto w-full max-w-3xl p-6">
            <PageHeader :title="query.company || query.contact_name" :subtitle="`Received ${new Date(query.created_at).toLocaleString('en-GB')}`">
                <template #actions>
                    <Link
                        ...
                        class="flex h-10 items-center gap-2 rounded-xl border border-zinc-200 px-4 text-[13px] font-medium text-zinc-700 transition-colors hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-300 dark:hover:bg-zinc-800"
                    >
            ...
            <div class="grid gap-4 rounded-2xl border border-zinc-200 bg-white p-6 sm:grid-cols-2 dark:border-zinc-800 dark:bg-zinc-900">
                <div v-for="f in fields" :key="f.label" class="flex items-start gap-3">
                    ...
                    <div>
                        <div class="text-[12px] font-medium text-zinc-400">{{ f.label }}</div>
            ...
            <div class="mt-4 rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
                <div class="mb-2 text-[12px] font-medium text-zinc-400">Message</div>
                <p class="whitespace-pre-wrap leading-relaxed text-zinc-700 dark:text-zinc-200">{{ query.message }}</p>
            </div>

            <a
                :href="`mailto:${query.email}`"
                class="mt-6 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#8e2527] font-medium text-white transition-colors hover:bg-[#a32c2f]"
            >
