...
# task every 5 minutes, so the crawl survives net drops, crashes and reboots:
# each tick resumes every shard from its own page cursor. Nothing banked is
# ever re-fetched.
#
# SPEED MODEL: ~3,700 search pages split into 4 parallel scrape shards, each
# with a big concurrent proxy pool. The image warm runs IN PARALLEL with
# scraping (scrape hits AutoTrader via proxies; warm hits the Bunny CDN), so
# the two phases overlap instead of stacking.
# Each tick:
#   1. mysqld up + 512M buffer pool
...
#   5. finish when all shards done AND warm caught up -> done + self-delete
#   6. write a live status page
...
$warmMk  = Join-Path $state 'warm-atwarm.done'
...
& 'C:\xampp\mysql\bin\mysql.exe' -h 127.0.0.1 -P 3307 -u root -B -e 'SET GLOBAL innodb_buffer_pool_size=7516192768' 2>$null
...
$refreshing = Get-CimInstance Win32_Process -Filter "Name = 'php.exe'" | Where-Object { $_.CommandLine -like '*scrape:refresh-proxies*' }
if ($needProxies -and -not $refreshing) {
    Start-Process -FilePath $php -ArgumentList (@('artisan','scrape:refresh-proxies','--validate','--limit=3200','--pool=250','--timeout=6') -join ' ') -WorkingDirectory $project -WindowStyle Hidden
    Log 'refreshing proxy pool'
}

# 3. image warm IN PARALLEL with scraping. While scraping continues, a caught-up
#    marker is stale (more cars coming) so clear it and relaunch.
...
if (-not $allShardsDone -and (Test-Path $warmMk)) { Remove-Item $warmMk -Force -ErrorAction SilentlyContinue }
if (-not $warming -and -not (Test-Path $warmMk)) {
    Start-Process -FilePath $php -ArgumentList (@('artisan','products:warm-cdn','--website=autotraderza','--shard=atwarm','--pool=60','--timeout=30') -join ' ') -WorkingDirectory $project -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logDir 'scrape-imagewarm.log') -RedirectStandardError (Join-Path $logDir 'scrape-imagewarm.err.log')
...
if (-not $allShardsDone) {
    foreach ($s in $shards) {
        ...
        if (-not $running) {
            $sargs = @('artisan','scrape:autotrader','--deep',('--shard=' + $s.name),('--min-page=' + $s.min),('--max-page=' + $s.max),('--pool=' + $pool),("--proxy-file=$proxies"),'--usd-rate=0.055','--delay-ms=0')
            Start-Process -FilePath $php -ArgumentList ($sargs -join ' ') -WorkingDirectory $project -WindowStyle Hidden -RedirectStandardOutput (Join-Path $logDir ('scrape-' + $s.name + '.log')) -RedirectStandardError (Join-Path $logDir ('scrape-' + $s.name + '.err.log'))
...
# 6. aggregate per-shard progress into a status page
...
$etaH = if ($rate -gt 0) { [math]::Round((92807 - $tot) / $rate / 60, 1).ToString() + ' h' } else { '-' }
$warmState = if ($allShardsDone) { if (Test-Path $warmMk) { 'complete' } else { 'finishing' } } else { 'parallel' }
$lines = @()
$lines += '<!doctype html><meta charset="utf-8"><title>AutoTrader scrape</title><meta http-equiv="refresh" content="30">'
$lines += '<style>body{font-family:Segoe UI,system-ui,sans-serif;background:#0b1e3b;color:#fff;margin:0;display:grid;place-items:center;min-height:100vh}.card{background:#122c53;border-radius:18px;padding:34px 40px;width:min(600px,92vw)}h1{font-size:18px;color:#9db2d4}.big{font-size:44px;font-weight:800}.bar{height:14px;background:#0b1e3b;border-radius:10px;overflow:hidden;margin:10px 0 22px}.fill{height:100%;background:linear-gradient(90deg,#e01f26,#ff5a60)}.grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px}.k{font-size:26px;font-weight:800}.l{font-size:12px;color:#9db2d4}</style>'
$lines += '<div class=card><h1>AUTOTRADER SCRAPE - ' + $doneCount + '/4 SHARDS DONE - WARM ' + $warmState.ToUpper() + '</h1>'
$lines += '<div class=big>' + $pct + '%</div><div class=bar><div class=fill style="width:' + $pct + '%"></div></div><div class=grid>'
$lines += '<div><div class=k>' + $tot + '</div><div class=l>products / ~92807</div></div>'
$lines += '<div><div class=k>' + $img + '</div><div class=l>images</div></div>'
$lines += '<div><div class=k>' + $etaH + '</div><div class=l>est remaining</div></div>'
$lines += '<div><div class=k>' + [math]::Round($rate,0) + '</div><div class=l>products/min</div></div>'
$lines += '<div><div class=k>' + $liveProxies + '</div><div class=l>live proxies</div></div>'
$lines += '<div><div class=k>' + $fail + '</div><div class=l>failures</div></div>'
$lines += '</div><div class=l style="margin-top:18px">updated ' + (Get-Date -Format o) + '</div></div>'
Set-Content -Path (Join-Path $state 'autotrader-status.html') -Value ($lines -join "`n") -Encoding utf8
